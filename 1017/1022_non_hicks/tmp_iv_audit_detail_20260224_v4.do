...
log using "$RES_LOG/iv_audit_detail_v4_`RUN_TAG'.log", text replace
...
foreach G in G1_17_19 G2_39_41 {
    ...
    capture drop phi_lag klag mlag xlag llag ksqlag lsqlag lages lages2q klages shat_lag l_ind_yr k_ind_yr m_ind_yr lnagelag
    ...
    drop if missing(const, r, l, k, phi, phi_lag, llag, klag, mlag, lsqlag, ksqlag, ///
        ...
        lnagelag, firmcat_2, firmcat_3, dy2002, dy2003, dy2004, dy2005, dy2006, dy2007, klages)
    ...
    foreach S in A B C {
        ...
        foreach z of local z_excl {
            ...
            if _rc {
                ...
                if _rc {
                    local miss 1
                    post `H' ("`G'") ("`S'") ("exist") ("`z'") ("non-numeric") (`Nsample') (.) (.) (.) (.) (.)
                }
                else {
                    quietly count
                    local NN = r(N)
        ...
        if `miss' {
            post `H' ("`G'") ("`S'") ("ivreg2") (".") ("skipped: var check failed") (`Nsample') (.) (.) (.) (.) (.)
            continue
        }
...
save "$RES_DATA/iv_audit_detail_v4.dta", replace
export delimited using "$RES_DATA/iv_audit_detail_v4.csv", replace
