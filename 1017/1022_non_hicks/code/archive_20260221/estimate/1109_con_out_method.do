...
if (g=="G1_17_19") {
   ...
   Z = st_data(., ("const","firmcat_2","firmcat_3", "dy2002","dy2003","dy2004","dy2005","dy2006","dy2007","lsqlag", "ksqlag", "llag","klag","mlag","lages","lages2q","klages","mlagxlag"))
   ...
   Z = st_data(., ("const","firmcat_2","firmcat_3", "dy2002","dy2003","dy2004","dy2005","dy2006","dy2007","lsqlag", "ksqlag", "llag","klag","mlag","lages","lages2q","klages","mlagxlag"))
    ...
    // 4) ←←← 就在这里：标准化 Z（Mata 里执行；不要在 Stata 层执行）
...
void GMM_DL_weighted(todo, b, crit, g, H)
{
  ...
  // --- 早停：目标值极小就直接告诉优化器"梯度=0"，避免继续 line-search ---
    ...
    /* 数值梯度：相对步长 + 中心差分（行向量） */
// --- 数值梯度：已知 cols(CONSOL)==1，因此 PO=(C, Oel) ---
if (todo>=1) {
...
void run_two_step()
{
    ...
    //（C）用"特征值地板"替代原来的 ridge：S = V diag(d) V', 把过小/负的 d 拉到 τ
    real matrix V
    real colvector d
    symeigensystem(S, V, d)    // 对称特征分解，数值更稳
    real scalar tau
    tau = 1e-4                 // 建议 Z 先标准化时取 1e-4~1e-3

    d = (d :> tau) :* d :+ (d :<= tau) :* tau   // d_j = max(d_j, τ)

    //（D）Wg = S^{-1} = V diag(1./d) V'（保证正定且条件数有上界）
    Wg = V * diag(1 :/ d) * V'
...
optimize_init_nmsimplexdeltas(S2, 0.05)      // 初始单纯形步长，按需 0.01~0.1 调
optimize_init_conv_maxiter(S2, 400)
...
// 诊断：每个工具的样本矩 gbar_k = mean(z_k * XI2_local) 及标准化贡献 

// 诊断：每个工具的样本矩 gbar_k 及标准化贡献
real scalar N
real colvector gbar, gbar_std, sdS   // ← 用 colvector

N        = rows(Z)
gbar     = quadcross(Z, XI2_local) :/ N
sdS      = sqrt(diagonal(S)) :+ 1e-12   // ← 不转置，保持 Kz×1
gbar_std = gbar :/ sdS                   // ← 维度匹配

    st_matrix("gbar_byZ",     gbar)                 // ← 写回到 Stata
    st_matrix("gbar_std_byZ", gbar_std)
    printf(">> [Mata] wrote gbar_byZ (%g×1) and gbar_std_byZ.\n", rows(gbar))
