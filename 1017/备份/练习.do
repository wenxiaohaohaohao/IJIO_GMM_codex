...
**去除掉少于两年的公司
bys firmid: gen nobs = _N       // 每家企业的样本期数
keep if nobs >= 2               // 只保留出现两年及以上的企业
drop nobs
...
* 3. 识别"新连续段"的开始
by firmid: gen seqblock = (gap != 1 | gap == .)
by firmid: replace seqblock = sum(seqblock)

* → seqblock 表示企业内部的连续段编号
*   例如 firmid=241: 年份[2000,2003,2006] → seqblock = 1,2,3
...
* 7. 检查结果
bys firmid: egen nobs_firm = count(year)
tab nobs_firm
*************************生成滞后项
...
if (g=="G1_17_19") {
    // 原先 anchor=18 的 IV（Kz=9）
	   Z = st_data(., ("const","firmcat_2","firmcat_3", "dy2002","dy2003","dy2004","dy2005","dy2006","dy2007","lsqlag", "ksqlag", "llag","klag","mlag","lages","lages2q","klages","llag2"))
