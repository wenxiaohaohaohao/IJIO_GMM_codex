...
void run_two_step()
{
    ...
    v = Wg_opt * m_opt           // Kz×1
    J_contrib = N :* (m_opt :* v)  // 每个 IV 的贡献，元素乘
...
* ---------【这里插 IV 诊断块】---------
* 线性 IV 诊断（只做诊断，不当作最终估计）
* 注意：需要先 ssc install ivreg2
capture ssc install ranktest, replace
capture ssc install ivreg2, replace


* 核心IV回归命令（Group 1: G1_17_19）
ivreg2 r (l k lsq ksq m es es2q = ///
           llag klag mlag          /// 滞后项（经典IV）
           lages klages            /// 滞后与年龄交互项
           l_ind_yr k_ind_yr m_ind_yr /// Bartik型行业-年度均值（核心）
           /// 外部宏观IV（可选，若无则删除此行）
         ) ///
       dy2002-dy2007 lnage firmcat_2 firmcat_3, ///
       robust cluster(firmid) first endog(/*指定内生变量*/)


* （看完 first / weakid / overid 的结果，再决定删哪几个 IV）
