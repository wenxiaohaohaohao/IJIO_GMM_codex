...
if _rc global ROOT "D:/paper/IJIO_GMM_codex_en/1017/1022_non_markdown_non_hicks"
...
gen S = importint/(domesticint+importint)
...
* phi cubic (unchanged)
reg r c.(l k m x pi wx wl S2q)##c.(l k m x pi wx wl S2q)##c.(l k m x pi wx wl S2q) ///
...
gen double lagS   = L.S
gen double lagS2q = L.S2q
*gen double laglage = L.lnage
gen double klagS2q=k*lagS2q
*gen double lagft   = L.firmtype
*gen double laglmana= L.lnmana
...
drop if missing(r,l,k,phi,phi_lag,llag,mlag,ksqlag,lsqlag,klag,S2q,lagS2q,lnage,firmtype)
...
void refresh_globals()
{
    ...
    X       = st_data(., ("const","l","k","lsq","ksq","m","S2q"))
    X_lag   = st_data(., ("const","llag","klag","lsqlag","ksqlag","mlag","lagS2q"))
...
if (g=="G1_17_19") {
    ...
    Z = st_data(., ("const","llag","k","klag","l","lsq","mlag","lagS2q","mlagk"))
	printf(">> Using IV: IV_anchor18_Kz9\n")
}
else  {
    // 原先 anchor=40 的 IV（Kz=9）
    Z = st_data(., ("const","llag","k","mlag","klag","l","lagS2q","llagklag","lsq","mlagk", "llagmlag" ))
...
tempfile bootres
local failed = 0
postfile `H' rep double(b_const b_l b_k b_lsq b_ksq b_m b_S2q ) using "`bootres'", replace
...
forvalues r = 1/`B' {
    preserve
       ...
       * mata: st_local("GROUPNAME","`GROUPNAME'")
        ...
        // 只在收敛时 post；未收敛跳过
        if (_rc==0 & scalar(gmm_conv)==1) {
            post `H' (`r') (r(b_c1)) (r(b_l)) (r(b_k)) (r(b_lsq)) (r(b_ksq)) (r(b_m)) (r(b_S2q))
...
order group b_const se_const b_l se_l b_k se_k b_lsq se_lsq b_ksq se_ksq b_m se_m b_S2q se_S2q  J_unit J_opt J_df J_p N
compress
save "$DATA_WORK/gmm_point_group_`GROUPNAME'.dta", replace
