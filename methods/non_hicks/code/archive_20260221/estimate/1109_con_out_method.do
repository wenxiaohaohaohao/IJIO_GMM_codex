...
if (g=="G1_17_19") {
    // 鍘熷厛 anchor=18 鐨?IV锛圞z=9锛?   Z = st_data(., ("const","firmcat_2","firmcat_3", "dy2002","dy2003","dy2004","dy2005","dy2006","dy2007","lsqlag", "ksqlag", "llag","klag","mlag","lages","lages2q","klages","mlagxlag"))
	printf(">> Using IV: IV_anchor18_Kz9\n")
}
else  {
    // 鍘熷厛 anchor=40 鐨?IV锛圞z=9锛?   Z = st_data(., ("const","firmcat_2","firmcat_3", "dy2002","dy2003","dy2004","dy2005","dy2006","dy2007","lsqlag", "ksqlag", "llag","klag","mlag","lages","lages2q","klages","mlagxlag"))
	printf(">> Using IV: IV_anchor40_Kz9\n")
}

    // 4) 鈫愨啇鈫?灏卞湪杩欓噷锛氭爣鍑嗗寲 Z锛圡ata 閲屾墽琛岋紱涓嶈鍦?Stata 灞傛墽琛岋級
...
// --- 鏁板€兼搴︼細宸茬煡 cols(CONSOL)==1锛屽洜姝?PO=(C, Oel) ---
if (todo>=1) {
...
void run_two_step()
{
    ...
    //锛圕锛夌敤"鐗瑰緛鍊煎湴鏉?鏇夸唬鍘熸潵鐨?ridge锛歋 = V diag(d) V', 鎶婅繃灏?璐熺殑 d 鎷夊埌 蟿
    real matrix V
    real colvector d
    symeigensystem(S, V, d)    // 瀵圭О鐗瑰緛鍒嗚В锛屾暟鍊兼洿绋?    real scalar tau
    tau = 1e-4                 // 寤鸿 Z 鍏堟爣鍑嗗寲鏃跺彇 1e-4~1e-3

    d = (d :> tau) :* d :+ (d :<= tau) :* tau   // d_j = max(d_j, 蟿)

    //锛圖锛塛g = S^{-1} = V diag(1./d) V'锛堜繚璇佹瀹氫笖鏉′欢鏁版湁涓婄晫锛?    Wg = V * diag(1 :/ d) * V'
...
optimize_init_nmsimplexdeltas(S2, 0.05)      // 鍒濆鍗曠函褰㈡闀匡紝鎸夐渶 0.01~0.1 璋?optimize_init_conv_maxiter(S2, 400)
...
// 璇婃柇锛氭瘡涓伐鍏风殑鏍锋湰鐭?gbar_k = mean(z_k * XI2_local) 鍙婃爣鍑嗗寲璐＄尞 

// 璇婃柇锛氭瘡涓伐鍏风殑鏍锋湰鐭?gbar_k 鍙婃爣鍑嗗寲璐＄尞
real scalar N
real colvector gbar, gbar_std, sdS   // 鈫?鐢?colvector

N        = rows(Z)
gbar     = quadcross(Z, XI2_local) :/ N
sdS      = sqrt(diagonal(S)) :+ 1e-12   // 鈫?涓嶈浆缃紝淇濇寔 Kz脳1
gbar_std = gbar :/ sdS                   // 鈫?缁村害鍖归厤

    st_matrix("gbar_byZ",     gbar)                 // 鈫?鍐欏洖鍒?Stata
    st_matrix("gbar_std_byZ", gbar_std)
    printf(">> [Mata] wrote gbar_byZ (%g脳1) and gbar_std_byZ.\n", rows(gbar))
...
**********************************鏌ョ湅宸ュ叿鍙橀噺鐨勬湁鏁堟€?mat list gbar_byZ,     format(%9.6f)
mat list gbar_std_byZ, format(%9.6f)
