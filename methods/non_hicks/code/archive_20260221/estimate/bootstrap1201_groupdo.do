...
void refresh_globals()
{
    ...
    if (g=="G1_17_19") {
        // 鍘熷厛 anchor=18 鐨?IV
		Z = st_data(., ("llag", "klag", "mlag", "lages", "klages","mlagk", "l_ind_yr", "k_ind_yr", "m_ind_yr" ))   
        printf(">> Using IV: IV_anchor18_Kz9\n")
    }
    else  {
        // 鍘熷厛 anchor=40 鐨?IV
         Z = st_data(., ("const","llag","k","klag","l","lages","mlagk","mlagl","lsq"))
...
void run_two_step()
{
    ...
    optimize_init_evaluatortype(S2, "d0")      // 鐢ㄦ暟鍊兼搴?鈥斺€?120娣诲姞 鈥斺€?*
    ...
    // 鏍锋湰骞冲潎鐭?    m_unit = quadcross(Z, XI1) :/ N
    ...
    // ===== 鍒嗚В J 鍒版瘡涓?IV 涓?=====
    real colvector v, J_contrib
    v = Wg_opt * m_opt           // Kz脳1
    J_contrib = N :* (m_opt :* v)  // 姣忎釜 IV 鐨勮础鐚紝鍏冪礌涔?
    // 瀛樺埌 Stata锛屾柟渚?inspect
    st_matrix("J_contrib", J_contrib')
...
* ---------銆愯繖閲屾彃 IV 璇婃柇鍧椼€?--------
* 绾挎€?IV 璇婃柇锛堝彧鍋氳瘖鏂紝涓嶅綋浣滄渶缁堜及璁★級
* 娉ㄦ剰锛氶渶瑕佸厛 ssc install ivreg2
capture ssc install ranktest, replace
capture ssc install ivreg2, replace

* 鏍稿績IV鍥炲綊鍛戒护锛圙roup 1: G1_17_19锛?ivreg2 r (l k lsq ksq m es es2q = ///
           llag klag mlag          /// 婊炲悗椤癸紙缁忓吀IV锛?           lages klages            /// 婊炲悗涓庡勾榫勪氦浜掗」
           l_ind_yr k_ind_yr m_ind_yr /// Bartik鍨嬭涓?骞村害鍧囧€硷紙鏍稿績锛?           /// 澶栭儴瀹忚IV锛堝彲閫夛紝鑻ユ棤鍒欏垹闄ゆ琛岋級
         ) ///
       dy2002-dy2007 lnage firmcat_2 firmcat_3, ///
       robust cluster(firmid) first endog(/*鎸囧畾鍐呯敓鍙橀噺*/)

* 锛堢湅瀹?first / weakid / overid 鐨勭粨鏋滐紝鍐嶅喅瀹氬垹鍝嚑涓?IV锛?
...
matrix list J_contrib

// 鎶婃瘡涓?IV 鐨?m_opt 鍜?J_contrib 鎵撳嵃鍑烘潵
local Znames const llag k mlag klag l lsq lages mlagk klages

forvalues j = 1/10 {
    local zname : word `j' of `Znames'
    
    // 浠庣煩闃典腑鍙栧嚭褰撳墠 IV 鐨勭煩鍊煎拰璐＄尞
    scalar m_j  = moments_opt[1, `j']
    scalar J_j  = J_contrib[1, `j']
    
    di as txt "`zname':  m_opt = " %9.5f m_j ///
        "   J_part = " %9.5f J_j ///
        "   share = " %6.3f (J_j / J_opt)
}
