...
preserve
    ...
    * =============================================================== *
    *   DECOMPOSITION TABLE 鈥?PATH (constant lines) + SHAPLEY         *
    ...
    * -------- Nicely formatted table in Stata only --------
    ...
    label var dS               "螖 Actual"
    label var md_path          "螖 via Const-MD path (a鈫抋1; b0,t0)"
    label var tech_path        "螖 via Const-Tech path (t鈫抰1; a0,b0)"
    label var inter_path       "螖 interaction (residual)"
    ...
    di as txt "{hline 120}"
    di as txt "Foreign-input cost share decline 鈥?PATH (constant lines with b0,t0 for Const-Tech) vs SHAPLEY"
    ...
    * ---- Plot per sector (solid lines; one-line small legend)
    ...
    foreach s of local sectors {
        ...
        * Safe filename token
        ...
        graph export "$RES_FIG/counterfactualshare_`sectorfn'.png", replace width(1200)
    }
restore
