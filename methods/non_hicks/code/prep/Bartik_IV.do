...
* (1) Z_HHI_post  = HHI0_country 脳 post2002
* (2) Z_tariff    = sum_h share0(h) 脳 tariff(h,t) /100
...
preserve
    ...
    * 濡傛灉鏌愪簺 firm 鍦ㄥ熀鏈熸病鏈?country锛堟瀬灏戞暟锛夛紝HHI0_country 鍙兘缂哄け锛?    replace HHI0_country = . if missing(HHI0_country)
...
preserve
    ...
    * 濡傛灉 using 鏁版嵁閲?year-hs6 涓嶅敮涓€锛氱敤鍧囧€艰仛鍚堟垚鍞竴
...
* share0 缂哄け琛ㄧず"鍩烘湡娌′拱杩囪 hs6"锛屾潈閲?0
...
label var Z_tariff "Baseyear hs6 share 脳 tariff path (scaled by /100)"
...
* 杈撳嚭涓€涓粺涓€鐨?firm-year IV 鏂囦欢
save "$DATA_RAW/firm_year_IVs_Ztariff_ZHHI.dta", replace
****************************************************
* DONE
****************************************************
