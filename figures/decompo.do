...
gen counterfactualshare = markdowncd1/(markdowncd1*markdownnonHicks + mdcd3)
...
preserve
    ...
    * Consistent ACTUAL from aggregated components (ratio of weighted means)
    gen cshare_actual = w_markdowncd1 / ( w_markdowncd1*w_markdownnonHicks + w_mdcd3 )
    ...
    * Counterfactuals (same functional form, different components fixed)
    gen cshare_constTech = w_markdowncd1 / ( w_markdowncd1*w_markdownnonHicks_base + w_mdcd3 )
    label var cshare_constTech "Constant Technology"

    gen cshare_constMD   = w_markdowncd1_base / ( w_markdowncd1_base*w_markdownnonHicks + w_mdcd3_base )
