...
* compute mean by cic2 and year (one obs per cic2-year)
bysort cic2 year: egen mean_taylor = mean(markdownTaylor)
bysort cic2 year: egen mean_nonhicks = mean(markdownnonHicks)
bysort cic2 year: keep if _n==1

* drop years where both medians missing
drop if missing(mean_taylor) & missing(mean_nonhicks)

* Create an individual graph for each sector (yearly means)
foreach s of local sectors {
    ...
    * 单独导出每个行业的图形
    graph export "sector_`s'_markdown_mean.png", replace width(1200)
}
